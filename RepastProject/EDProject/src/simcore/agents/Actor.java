...
public class Actor extends Agent {
	...
	public void ExecMission() {
		...
		if (stepLogic instanceof MoveAction) {
			...
			// decide what instance of that room
...
//				System.out.println(fields.get(i));
	...
	/*
	 * ATTENTION!!!! This function below is a failed attempt, which tries to invoke
	 * routing function in each move step then get a stack, and pop its Top one
	 * point as next destination.
	 * 
	 * Agents may stuck in this way so please solve the problem by updating the
	 * routing method.
	 * 
	 * Although setting a routing stack then move step by step following it can be
	 * implemented, we do need a dynamic routing function to follow a movable
	 * target. The demand of our routing function is that, find a way between two
	 * Objects in our ED, and set next step of it as target to move towards it, only
	 * next step instead of the whole route. Given a target, return only one
	 * NextStepPoint, and by loop the same execution we are finally able to reach
	 * the destination.
	 * 
	 * an EDMap class is used for agents to find way to destination, which calls
	 * your routing function.
	 * 
	 * BTW I think maybe we can improve a generated [Grid] route then use a
	 * [ContinuousSpace] way to move it to make it looks better.
	 * 
	 * 
	 * public void MoveTowards(Object o) {
	 * 
	 ...
	 * 
	 * 
	 * System.out.println("next target: "); if(!s2.isEmpty()) {
	 * System.out.println(s2.peek()); }else {
	 * System.out.println("RoutePlan is Empty"); }
	 * 
	 * 
	 * System.out.println("routePlan: "); while(!s2.isEmpty()) { GridPoint ggp =
	 * s2.pop(); System.out.println(ggp);
	 * 
	 ...
	 * 
	 * 
	 * if(!route.isEmpty()) { if(SpaceAtByGrid(route.peek())) { route.pop(); }
	 * GridPoint pointOfTarget = route.pop(); MoveTowards(pointOfTarget); }else {
	 * System.out.println("RoutePlan is Empty"); }
	 * 
	 * 
	 * 
...
//			myPoint = space.getLocation(this);
//			grid.moveTo(this, (int) myPoint.getX(), (int) myPoint.getY());
//			Iterable<Object> plstItemsHere = grid.getObjectsAt((int) myPoint.getX(), (int) myPoint.getY());
//			
//			List<Object> actualList = new ArrayList<Object>();
//			plstItemsHere.forEach(actualList::add);
//			
//			ArrayList<Object> plstWalls = (ArrayList<Object>) actualList.stream().filter(x -> x instanceof Wall).collect(Collectors.toList());
//			
//			if(plstWalls.size() > 0) {
