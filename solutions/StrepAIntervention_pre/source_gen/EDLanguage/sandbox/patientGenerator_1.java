...
public class patientGenerator_1 extends AgentGenerator {
  ...
  public void generateAttribute_a_0(patient agent) {
    ...
    runningTotal = (RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusSusceptible"));
    if (rndDouble < (runningTotal / 100)) {
      agent.StrepAInfectionStatus = "Susceptible";
      return;
    }
    if (rndDouble < ((RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusAsymptomatic") + runningTotal) / 100)) {
      agent.StrepAInfectionStatus = "Asymptomatic";
      runningTotal += RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusAsymptomatic");
      return;
    } else {
      runningTotal += RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusAsymptomatic");
    }
    if (rndDouble < ((RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusSymptomatic") + runningTotal) / 100)) {
      agent.StrepAInfectionStatus = "Symptomatic";
      runningTotal += RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusSymptomatic");
      return;
    } else {
      runningTotal += RunEnvironment.getInstance().getParameters().getDouble("StrepAInfectionStatusSymptomatic");
    }
  ...
  public void generateAttribute_b_0(patient agent) {
    ...
    runningTotal = (RunEnvironment.getInstance().getParameters().getDouble("admissionRouteElectiveattendance"));
    if (rndDouble < (runningTotal / 100)) {
